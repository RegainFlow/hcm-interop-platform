        ...
        <div key={index} className="glass-card p-5 relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start mb-4 relative z-10">
            <div className={`p-2 rounded-lg bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)]`}>
              <metric.icon size={24} className="text-[#00d6cb] drop-shadow-[0_0_5px_rgba(0,214,203,0.5)]" />
            </div>
            <span className={`text-xs font-mono py-1 px-2 rounded-full border ${
              metric.isPositive 
          ...
          <div className="relative z-10">
            <h3 className="text-[#a6a6a6] text-sm font-medium mb-1">{metric.title}</h3>
            <p className="text-2xl font-bold font-primary text-white tracking-tight">{metric.value}</p>
          </div>
        </div>
      ))}
    </div>
